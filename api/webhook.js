// api/webhook.js — V100: pregunta de ciudad separada por el mismo canal del copy
...
function buildFriendlyCityQuestion() {
  const index = Math.floor(Math.random() * CITY_QUESTIONS_FRIENDLY.length);
  return CITY_QUESTIONS_FRIENDLY[index] || CITY_QUESTIONS_FRIENDLY[0];
}
...
/**
 * Envía un segundo mensaje por el mismo endpoint que usa la plantilla.
 * Se usa para la pregunta de ciudad después del copy + imagen.
 */
async function enviarPreguntaCiudadSeparada(userId, from, text) {
  const msg = clean(text);
  if (!msg) return false;
  ...
  try {
    ...
    if (!response.ok) {
      console.error(
        "❌ No se pudo enviar la pregunta separada:",
        response.status,
        responseText
      );
      return false;
    ...
    if (responseJson && responseJson.ok === false) {
      console.error(
        "❌ send-whatsapp respondió ok=false para pregunta de ciudad:",
        responseJson
      );
      return false;
    }
    ...
    console.log("✅ Pregunta de ciudad enviada como segundo mensaje:", msg);
    return true;
  } catch (error) {
    console.error("❌ Error enviando pregunta separada:", error);
    return false;
  }
}
...
async function enviarPlantillaCompleta({ userId, from, templateName, fallbackText, productNameFallback, resetOrderForNewPresentation = false }) {
  ...
  if (
    ...
    preguntaCiudadSeparada = buildFriendlyCityQuestion();
  }
  ...
  if (buttons && buttons.length > 0) {
    ...
    if (preguntaCiudadSeparada) {
      await sleep(1200);
      await enviarPreguntaCiudadSeparada(
        userId,
        from,
        preguntaCiudadSeparada
      );
  ...
  if (preguntaCiudadSeparada) {
    if (mensajeFinal || imagenes.length > 0) await sleep(1200);
    await enviarPreguntaCiudadSeparada(
      userId,
      from,
      preguntaCiudadSeparada
