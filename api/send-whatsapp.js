...
async function enviarPlantillaCompleta({ userId, from, templateName, fallbackText }) {
  ...
  if (buttons && buttons.length > 0) {
    ...
    // 🔧 CORREGIDO: Construir URL ABSOLUTA para llamar al mismo dominio
    // En Vercel, usar VERCEL_URL; en desarrollo, usar localhost
    let baseUrl = '';
    
    // Intentar obtener la URL base de varias formas
    if (process.env.VERCEL_URL) {
      ...
      // Fallback: intentar determinar desde el request (no disponible aquí)
