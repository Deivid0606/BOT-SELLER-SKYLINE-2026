...
async function metaSendInteractiveWithImage(config, to, text, buttons, imageUrl) {
  try {
    console.log('📸 Enviando INTERACTIVO CON IMAGEN:', imageUrl);
    console.log('📸 Botones recibidos:', JSON.stringify(buttons, null, 2));
    ...
    // 🔧 CORREGIDO: Limitar a 3 botones y formatear correctamente
    const maxButtons = buttons.slice(0, 3);
    
    const formattedButtons = maxButtons.map((btn, index) => {
      // Asegurar que el label sea string y tenga máximo 20 caracteres
      const label = String(btn.label || btn.text || `Opción ${index + 1}`)
    ...
    console.log('📸 Botones formateados:', JSON.stringify(formattedButtons, null, 2));
    ...
    console.log('📤 Payload COMPLETO a WhatsApp:', JSON.stringify(payload, null, 2));
    ...
    if (!r.ok) {
      console.error('❌ Meta interactive ERROR:', r.status, responseText);
      return false;
    }

    console.log('✅ Mensaje interactivo con imagen ENVIADO');
    console.log('📥 Respuesta Meta:', responseText);
    return true;
  } catch (err) {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.log('📥 ===== PAYLOAD RECIBIDO EN SEND-WHATSAPP =====');
    console.log('📥 to:', to);
    ...
    if (buttons && Array.isArray(buttons) && buttons.length > 0) {
      ...
      console.log('🎯 Botones:', JSON.stringify(buttons, null, 2));
